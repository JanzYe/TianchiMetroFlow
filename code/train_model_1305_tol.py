...
file_name = constants.DATA_PATH_MODELS + 'model_1305_tol.h5'
...
if mode == constants.TRAIN:
    ...
    train_x = input_x[2*constants.DATA_A_DAY:, :]
    train_y = input_y[2*constants.DATA_A_DAY:]

    n_samples = len(train_y)

    valid_x = np.reshape(input_x[-6*constants.DATA_A_DAY:-5*constants.DATA_A_DAY, :], (int(1*constants.DATA_A_DAY / n_outputs), sum(constants.FEAT_LEN) * n_outputs))
    valid_y = np.reshape(input_y[-6*constants.DATA_A_DAY:-5*constants.DATA_A_DAY], (int(1*constants.DATA_A_DAY / n_outputs), n_outputs))
else:
